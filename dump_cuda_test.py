...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    if checkpoint:
        (model_params, first_iter, train_idxs) = torch.load(checkpoint)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        breakpoint()
        data_tuple = (viewpoint_cam, model_params, opt, pipe, background, render_pkg, gt_image)
        torch.save(data_tuple, "/mnt/kostas-graid/sw/envs/wen/gaussian_test_tuple.pth")
        ...
        with torch.no_grad():
            ...
            if schema.num_views_to_add(iteration) > 0:
                #TODO: Add selector
                pass

            if (iteration in checkpoint_iterations):
                print("\n[ITER {}] Saving Checkpoint".format(iteration))
                torch.save((gaussians.capture(), iteration, scene.train_idxs), scene.model_path + "/chkpnt" + str(iteration) + ".pth")
